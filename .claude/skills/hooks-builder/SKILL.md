---
name: hooks-builder
description: Use when the user asks to design, build, optimize, or audit Claude Code HOOKS — event-driven shell commands in settings.json that fire on SessionStart, PreToolUse, PostToolUse, Stop, etc., to allow/block tools, inject context, or trigger side effects. Triggers — "build a hook", "add a hook", "run X every time I edit/commit/start a session", "block tool X when…", "hook into PreToolUse", "audit my hooks", or `/hooks-builder`. Sibling of `/skill-builder`, `/agent-builder`, `/routines-builder`, `/agents-team-builder`, `/plugin-builder` — that family picks the mechanism; this one is the event-driven-local specialist (the hooks lane `/routines-builder` hands off to). Runs a decision gate (hook vs routine vs skill-scoped hook vs plugin hook), a discovery interview (event, matcher, action, scope, failure mode), writes the hook script + the settings.json edit, then runs a MANDATORY supervised first-fire test. Hooks run arbitrary shell as YOU on every matched event — safety rails are not optional. Schema + protocol: `reference.md`.
