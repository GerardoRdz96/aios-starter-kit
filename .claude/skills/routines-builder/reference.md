...
- **Cloud routines bypass the *hook* half of the privacy gate.** The pre-push guard (over `context/`/`knowledge/`/`decisions/`/`artifacts/`) is a git hook on *your* machine, and a routine pushes from Anthropic's cloud, so it never fires there. The CI half (`.github/workflows/rails.yml`) does still run on the routine's push, but it runs *after* the content is already on the remote — it reports a leak, it does not prevent one. So keep personal dirs *out of the routine's repo scope* in the first place, and add **branch protection on `main`** requiring the `rails-guard` check, which is what turns the CI result into an actual block.
