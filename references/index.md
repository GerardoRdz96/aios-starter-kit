...
- [The data boundary](data-boundary.md) — run two AIOS instances, not one: a sensitive-work system restricted to approved models, and a general one for everything else. What goes where, how to set up the second, and the quiet failure mode a single careful system does not prevent. Read before wiring a connection that touches employer or client material.
